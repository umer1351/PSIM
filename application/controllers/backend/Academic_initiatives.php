...
class Academic_initiatives extends Backendcommon {
...
public function changeAcademicStatus($param = null){
		$objResponse = new xajaxResponse();
		if ($param != null) { 
			$slide_id = $param['slide_id'];
			$status = $param['status']; 
			$update_response = $this->page_model->changeAcademicStatus($slide_id,$status);
			if($update_response){
	...
	public function changeAcademicPoster($param = null){
		$objResponse = new xajaxResponse();
		if ($param != null) { 
			$slide_id = $param['slide_id'];
			$status = $param['status']; 
			$update_response = $this->page_model->changeAcademicPoster($slide_id,$status);
			if($update_response){
				$objResponse->script( "window.location.reload();");	
			}
		}
...
public function updateBottomAjax()
{
	...
	Public function privacy_policy()
	{
		...
		// 	die();
		// }
		// this page will be deleted and will be replaced by psimj page.
...
public function academic_initiatives($value='')
	{
		$this->data['moduleLink'] ="/3";
			$this->data['moduleName'] = "";
			...
			$data = $this->data;
			$data['listing']=$this->academic_initiative->getExamsListing();
			
			$data['common_data'] = $this->common_data;
			$template['body_content'] = $this->load->view('backend/academic-initiatives/index', $data, true);	
			$this->load->view('backend/layouts/template', $template, false);
	}


public function academic_initiatives_add($value='')
		{
		...
		$data['common_data'] = $this->common_data;
		$template['body_content'] = $this->load->view('backend/academic-initiatives/add', $data, true);			
		$this->load->view('backend/layouts/template', $template, false);
		}


public function addAcademicsAjax(){
		...
		if($image_upload==1){
			//Uploading Image
			$file_element_name = 'imgInp';  
			$config['upload_path'] = ASSET_FRONTEND_UPLOADS_PATH."academic";
			$config['allowed_types'] = 'jpg|png|jpeg|gif';
			/*$config['max_width'] = $width;
			$config['max_height'] = $height;
			$config['min_width'] = $width;
			...
			$image_name = $slide_image;
				...
				if($slide_id==0){
                    $qResponse = $this->page_model->addAcademicsSlide($deffered,$course_start_date,$expiry_date,$lifetime,$price,$image_name,$title,$details,$register_date,$top_information,$sub);
                    $msg_c = 'added';
                }else{
                        $qResponse = $this->page_model->updateAcademicsSlide($deffered,$course_start_date,$expiry_date,$lifetime,$price,$slide_id,$image_name,$title,$details,$register_date,$top_information,$sub);
                    $msg_c = 'updated';
                }
        if($qResponse){
			//Redirect to User profile page
	...
	public function addacademicDetails($value='')
		...
		$this->data['subModuleName'] = (isset($_GET['slide']) ? "Edit Slide":"Add Slide");
		$data = $this->data;
		$data['slide'] = (isset($_GET['slide']) ? $this->page_model->getacademicByID($_GET['slide']):false);
		$data['common_data'] = $this->common_data;
		$template['body_content'] = $this->load->view('backend/academic-initiatives/add_details', $data, true);			
		$this->load->view('backend/layouts/template', $template, false);
		}

		public function editacademicDetails($value='')
		{
		...
		$this->data['subModuleName'] = (isset($_GET['slide']) ? "Edit Slide":"Add Slide");
		$data = $this->data;
		$data['slide'] = (isset($_GET['module']) ? $this->page_model->getacademicDetailsID($_GET['module']):false);
		$data['common_data'] = $this->common_data;
		$template['body_content'] = $this->load->view('backend/academic-initiatives/edit_details', $data, true);			
		$this->load->view('backend/layouts/template', $template, false);
		}

		
		public function editacademicDetailsAjax(){
		$objResponse = new xajaxResponse();
		//Getting Variables	
		$slide_id = $_POST['slide_id'];
        ...
        $Speaker = $_POST['Speaker'];  
        $Details = $_POST['Details'];
        $qResponse = $this->page_model->editacademicDetailsAjax($topic,$slide_id,$Time,$Speaker,$Details);
		if($slide_id==0){
            // $qResponse = $this->page_model->addExamDetailsAjax($topic,$slide_id,$Time,$Speaker,$Details);
            $msg_c = 'added';
        }else{
