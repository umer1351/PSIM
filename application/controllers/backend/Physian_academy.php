...
class Physian_academy extends Backendcommon {
...
public function addExamsAjax(){
		...
		if($image_upload==1){
			...
			$image_name = $slide_image;
				...
				if($slide_id==0){
                    $qResponse = $this->page_model->addExamsSlide($image_name,$title,$details,$register_date,$top_information);
                    $msg_c = 'added';
                }else{
                        $qResponse = $this->page_model->updateExamsSlide($slide_id,$image_name,$title,$details,$register_date,$top_information);
                    $msg_c = 'updated';
                }
        if($qResponse){
			//Redirect to User profile page
