...
<div class="page-fixed-main-content" style="min-height: 380px">
	...
	<div class="col-md-12 col-xs-12 col-sm-12 performance-form">
		<div class="page-heading-related">
			<h3 class="form-section float-left-style"><?php echo ($slide ? 'Edit':'Add'); ?> Academics Details</h3>
		...
		<!-- BEGIN FORM-->
			<h4>Media Gallery</h4>
			<form action="<?php echo  BACKEND_ACADEMIC_MEDIA_AJAX ; ?>?exams_id=<?php echo $_GET['slide'] ?>" class="dropzone"></form>
			<?php 
			if(!empty($files)){ foreach($files as $row){ 
			?> 
			<a data-toggle="modal" onclick="update_status('<?php echo $row['id'] ?>','Image','changeAcademicImage','<?php echo DELETED_STATUS_ID ?>')" href="javascrpt:void(0)" class="btn btn-sm btn-circle btn-default"><img style="max-width: 100px;
			...
			<form action="<?php echo  BACKEND_ACADEMIC_POSTER_AJAX ; ?>?exams_id=<?php echo $_GET['slide'] ?>" class="dropzone"></form>
			<?php 
			if(!empty($poster)){ foreach($poster as $row){ 
			?> 
			<a data-toggle="modal" onclick="update_status('<?php echo $row['id'] ?>','Image','changeAcademicPoster','<?php echo DELETED_STATUS_ID ?>')" href="javascrpt:void(0)" class="btn btn-sm btn-circle btn-default"><img style="max-width: 100px;
			    max-height: 100px;" src="<?php echo ASSET_UPLOADS_FRONTEND_DIR.'exams/'.$row['file_name'] ?>"> Delete</a> 
